...
const Dashboard = () => {
	...
	const submitExchangeRate = async (event) => {
		...
		try {
			const response = await fetch('https://th3uour1u1.execute-api.us-east-2.amazonaws.com/devStage006/sponsorOrgs', {
				...
				body: JSON.stringify(exchangeRateData)// Send updated application data
			});
			...
			console.log('Exchange rate changed successfully:', result);  // Message from Lambda response
			alert('Exchange rate changed successfully!');
	...
	const renderContent = () => {
		switch (activeTab) {
			...
			case 'updateExchangeRate':
				return (
					<div>
					<h2>Point Exchange Rate</h2>
					<p>Edit the current point exchange rate for your organization.</p>
					<form>
					...
					onChange={changeExchangeData}
					/>

					<button type="button" onClick={submitExchangeRate}>
